...
if (window.location.pathname.includes('/auth/msa')) {
    ...
    throw new Error('[Xperience XCloud] Refreshing the page after logging in');
}
...
`);
    ...
    // Stop processing the script
    throw new Error('[Xperience XCloud] Executing workaround for Safari');
}

// Make sure it only run on /play
if (!window.location.pathname.match(/^\/[a-zA-Z]{2}-[a-zA-Z]{2}\/play/)) {
    throw new Error('[Xperience XCloud] Not xCloud page');
}
...
BxEventBus.Stream.on('dataChannelCreated', payload => {
    ...
    dataChannel.addEventListener('message', async (msg: MessageEvent) => {
        if (msg.origin === 'xperience-xcloud' || typeof msg.data !== 'string') {
            return;
        }
